...
+----+---------------------+------------------------------------------------------+-------------------------+
|  1 | 작심 독서실         | 0xE61000000101000000C194DCBC4CC85F40A934BD1F12B74240 | 작심 독서실 6층 입니다. |
|  2 | 지하철 8호선 모란역 | 0xE61000000101000000567F846140C85F4034773E4D77B74240 | 출 퇴근 합니다.         |
|  3 | 도미노피자 수진역점 | 0xE61000000101000000BE384C8F01C95F40CA6ABA9EE8B74240 | 도미노 맛있다.          |
|  4 | 잠실 IC             | 0xE61000000101000000F10005CD1DC65F40ACF58FCF75C24240 | 한번 찍어봅니다.        |
|  5 | MBR test            | 0xE61000000101000000E8A56263DEC85F40E7AD15C847B84240 | 반경 1km를 벗어납니다   |
|  6 | 반경 검색 641m      | 0xE6100000010100000081D2AB5C83C85F4050EA268CC1B74240 | 구글로 약 641m          |
+----+---------------------+------------------------------------------------------+-------------------------+
```
...
SELECT
    ...
    ST_DISTANCE(
        point, -- 저장된 좌표
        ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326) -- 파라미터: 현재 위치 좌표
    ) AS distance_in_meters,
    ST_Contains(ST_Buffer(ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326),642), point) as 'ST-642',
    ST_Contains(ST_Buffer(ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326),644), point) as 'ST-644',
    MBRContains(ST_Buffer(ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326),641), point) as '포함-MBR-641',
    MBRCoveredBy(ST_Buffer(ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326),641), point) as '경계선-MBR-641',
    MBRCoveredBy(ST_Buffer(ST_GeomFromText('POINT(37.4303521 127.1299667)', 4326),641.40), point) as '경계선-MBR-642'
FROM
    locations_mysql
```
#### 코드 설명  
+ ST_GeomFromText =  해당 좌표를 GIS를 4326 으로 지정합니다.
+ ST_Buffer 해당 좌표로부터 반지름 644 GIS(4326) 을 기준으로 원을 그립니다.
+ ST_Contains(geometry1, geometry2): 첫 번째 지오메트리가 두 번째 지오메트리를 실제로 포함하는지 여부를 평가합니다.
+ MBRCoveredBy(geometry1, geometry2): 첫 번째 지오메트리의 MBR이 두 번째 지오메트리를 포함하는지 여부를 평가합니다.

#### 결과
```SQL
+---------------------+-------------------------+--------------------+--------+--------+--------------+----------------+----------------+
| name                | description             | distance_in_meters | ST-642 | ST-644 | 포함-MBR-641 | 경계선-MBR-641 | 경계선-MBR-641.40 |
+---------------------+-------------------------+--------------------+--------+--------+--------------+----------------+----------------+
| 작심 독서실         | 작심 독서실 6층 입니다. | 27.937985822765814 |      1 |      1 |            1 |              0 |              0 |
| 지하철 8호선 모란역 | 출 퇴근 합니다.         |  342.8368926914897 |      1 |      1 |            1 |              0 |              0 |
| 도미노피자 수진역점 | 도미노 맛있다.          |  1189.856991108674 |      0 |      0 |            0 |              0 |              0 |
| 잠실 IC             | 한번 찍어봅니다.        | 10321.895262537773 |      0 |      0 |            0 |              0 |              0 |
